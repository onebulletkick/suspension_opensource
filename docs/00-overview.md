...
这不是资料索引，也不是软件教程集合。它是一套公开版悬架技术文档：读者应能沿着目标、轮胎、几何、簧上系统、仿真、校核、验证和复盘的顺序理解一套悬架为什么这样设计。
...
- 不要公开未经授权的队内资料、供应商资料、比赛文件、截图、表格或原始数据。
- 不清楚来源或授权时，先把内容改写成通用工程判断，并标注需要公开资料或测试数据继续验证。

## 参考来源

本仓库公开文档来自公开资料和经过改写的工程经验。公开参考资料、引用建议和来源处理边界见 [references.md](references.md)。
