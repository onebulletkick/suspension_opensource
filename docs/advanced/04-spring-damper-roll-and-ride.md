...
| `VR_d` | 阻尼速度比 damper velocity ratio | 无量纲 | 本章定义为阻尼器轴向速度除以轮端垂向速度 |
| `x_w` / `x_s` | 轮端垂向位移 / 弹簧位移 | mm 或 m | 必须说明正方向、测量点和是否为局部位移 |
| `F_z` | 轮胎垂向载荷 normal load | N | 正方向和数据来源要与轮胎章节一致 |

在第一次计算刚度和频率之前，先把以下术语分开写清楚：

| 名称 | 典型含义 | 容易混淆的点 |
| --- | --- | --- |
| spring rate | 弹簧本体刚度，通常在弹簧轴线方向定义 | 不能直接等于轮端刚度 |
| wheel rate | 轮心垂向等效刚度，受 motion ratio 和机构角度影响 | motion ratio 定义不同会导致平方项位置不同 |
| ride rate | 轮端刚度与轮胎刚度共同作用后的车身垂向等效刚度 | 轮胎刚度变化会改变实际 ride 感受 |
| roll stiffness | 车身侧倾刚度，由弹簧、防倾杆、轮距和几何共同影响 | 前后分配会影响轮载转移分配，不是越大越好 |
| damping ratio | 相对临界阻尼的无量纲描述 | 阻尼器实际力速曲线、温度和安装角会改变效果 |
...
建议在所有表格标题或页脚写明 convention：`MR_x = spring displacement / wheel vertical displacement`，`VR_d = damper shaft velocity / wheel vertical velocity`。如果同一项目里有人把 motion ratio 记成 wheel / spring，又有人把 velocity ratio 记成 damper / wheel，`MR^2` 和 `VR^2` 会在换算中互相倒置，最后表现为弹簧选型、阻尼器端力、行程裕度和结构载荷全部偏离。
...
若缺少可信的 `k_t`，不要为了让表格完整而伪造精度。可以先用保守范围做 sensitivity study，并在输出里标注“待验证：轮胎垂向刚度”。这类结果只能用来比较趋势，不能作为最终弹簧、车高或气动平台设定的放行依据。
...
阻尼器 damper 通过速度相关力控制弹簧和车身运动。阻尼比 damping ratio 是线性模型中常用的无量纲指标，但真实阻尼器通常具有低速、高速、压缩 compression、回弹 rebound、泄压、滞回、热衰退 heat fade 和调节机构等非线性特征。只给出一个名义阻尼比，不能说明阻尼器是否在实际工作区间内有效。
...
阻尼器工作区间的最低输出应包括：阻尼器速度-力曲线、轮端速度换算、悬架位移速度直方图、压缩/回弹调节方向、温升和热衰退风险、行程利用、限位策略和每次调校的记录格式。若无法获得可靠 damper dyno 曲线，应把阻尼器结论写成工程经验或待验证假设；若有台架条件，最好记录冷态、热态和重复 run 的曲线差异，避免把热衰退误判为调校方向。
...
非线性 motion ratio 的边界要写在图旁边：曲线覆盖的 wheel travel、转向角、左右轮同向或反向、是否含弹簧预载、是否含 bump rubber / helper spring、是否为 damper shaft 轴向速度。只有局部线性区可以用一个静态 MR 近似；行程末端、第三弹簧触发或摇臂角度快速变化时，应回到局部 tangent wheel rate 和阻尼速度比曲线。
...
防倾杆 anti-roll bar 的核心作用是改变前后侧倾刚度分配和弹性横向载荷转移分配，从而改变车辆平衡与调校窗口。它不会凭空创造轮胎抓地力；若因为更硬的防倾杆让某一轴的轮胎载荷更不均，而轮胎又有明显载荷敏感性，轴总抓地甚至可能下降。因此，防倾杆变化必须和胎温、胎压、轮荷、车手反馈、路面粗糙度和左右轮行程一起评审。

评审侧倾刚度分配时，建议把每次改动写成下面的问题，而不是写成“前硬不足、后硬过度”这类固定配方：

| 变量 | 必须一起看的证据 |
| --- | --- |
| 轮胎 load sensitivity | 前后轴轮载差、胎温分布、胎压变化、轮胎模型或测试窗口 |
| 前后轴载荷转移 | 弹性载荷转移、几何载荷转移、roll center 高度、轮距和质心高度 |
| anti-roll bar | 杆径/臂长/孔位或替换方案、左右轮耦合、单轮 bump 和路肩表现 |
| 气动车高和 pitch / roll | 底板、扩散器、前后翼或车身高度窗口，以及动态 ride height 变化 |
| 车手反馈 | 入弯、弯中、出弯、路肩、方向盘修正量和极限可预判性 |

如果某个防倾杆或侧倾分配让稳态圆数据变好，却让路肩、连续弯或低附着工况更难驾驶，这不是“数据和感觉谁对谁错”的问题，而是测试目标、轮胎状态、路面输入和模型边界需要一起复盘。
...
第三弹簧 third spring、heave spring、bump rubber、packers、辅助弹簧和限位块等附加弹性元件，常用于把 heave 起伏刚度、roll 侧倾刚度和 pitch 俯仰刚度部分解耦。公开资料中这部分证据多来自通用赛车、供应商教育材料或高下压力平台经验，能提供概念边界，但不足以证明 Formula Student 车辆必须采用这类方案。它们尤其常见于需要控制气动车高或在不同工况下改变等效刚度的平台，但也会显著增加调校和解释难度。

第三弹簧或 heave device 不应被写成高阶赛车的必备装备。只有当车辆确实需要围绕气动平台 aero platform、pitch / heave 控制、行程限位和包装空间建立独立调校逻辑时，才值得引入这类机构。若底板、扩散器、车架节点、摇臂空间、维护路径或传感器布置无法支持稳定验证，第三弹簧只会增加模型自由度和调校不确定性。

可以用下面的边界判断是否值得引入：

| 值得继续研究的条件 | 容易增加不确定性的条件 |
| --- | --- |
| 车辆对动态车高、heave 或 pitch 有明确气动/底板边界 | 主要问题其实来自主弹簧、防倾杆、阻尼、几何或胎压尚未收敛 |
| 已有可靠 motion ratio、行程、车高和位移传感器数据 | 缺少 damper dyno、悬架位移或车高数据，只能靠主观感觉解释 |
| 包装、车架节点、摇臂和维护路径能承受新增载荷路径 | 新机构让装配、检查、左右一致性或赛场维护显著变难 |
| 测试矩阵能区分 heave、roll、pitch、bump rubber 触发和轮胎状态 | 每次测试同时改多个变量，无法判断第三弹簧是否真正解决问题 |
...
簧上系统的软件任务是把弹簧、阻尼、运动比、侧倾和俯仰目标变成可复核的参数链。它不应只输出一个“最终刚度”，而要保留模型约定、轮胎刚度、运动比定义、阻尼工作区间和调校验证路径。

| 技术问题 | 推荐工具 | 输入 | 输出 | 传给下一步 | 验证方式 |
| --- | --- | --- | --- | --- | --- |
| 参数主表 | 表格、Git / Markdown | 角重、簧上质量、轮胎垂向刚度、目标偏频、运动比定义、阻尼器信息 | 刚度和偏频计算表、单位表、版本记录 | MATLAB / Python、Adams Car、测试记录 | 手算数量级、单位换算和参数来源复核 |
| 偏频与轮端刚度 | MATLAB / Python、表格 | `k_s`、`MR_x`、`k_w`、`k_t`、单角质量和目标姿态窗口 | 偏频、wheel rate、effective ride rate、敏感性图 | 弹簧选择、整车模型、调校范围 | 与表格、简化公式和实车静态压缩量交叉检查 |
| 运动比和布置 | Adams Car、CATIA / 3D CAD | 硬点、pushrod / pullrod、rocker、减振器行程、限位和包装边界 | motion ratio 曲线、减振器速度换算、干涉和维护空间说明 | 阻尼分析、结构载荷、制造检查 | 全行程、转向 / 侧倾组合、台架和位移传感器复核 |
| 阻尼方向分析 | MATLAB / Python、阻尼器数据处理工具 | 阻尼器速度-力曲线、轮端速度、测试或假想路面输入、压缩 / 回弹方向 | 低 / 中 / 高速区间、目标调节方向、速度直方图 | shakedown tuning、测试矩阵 | 用位移传感器、车高变化、轮胎动载和车手反馈复核 |
| 侧倾和俯仰参数 | MATLAB / Python、Adams Car、整车模型 | roll center、轮距、质心高度、侧倾刚度、制动 / 加速工况 | roll stiffness distribution、pitch response、轮荷变化趋势 | 仿真优化、测试计划、答辩证据 | 与轮胎载荷敏感性、稳态/瞬态模型和实车数据相关性检查 |
...
| 测试闭环记录 | 角重、车高、行程、damper dyno、悬架位移传感器、胎温胎压、车手反馈和 post-run inspection | 公开字段和证据链，具体原始数据留在项目记录中 |
...
| 台架 | 弹簧刚度、阻尼器速度-力曲线、防倾杆扭转刚度、第三弹簧触发 | 台架报告、校准记录、冷/热态曲线、温度说明 |
| 仿真 | 四分之一车、纵向 pitch、侧向 roll、整车参数扫描 | 模型版本、输入边界、敏感性结果 |
| 静态实车 | 车高、角重、预载、悬架行程、限位间隙、调节重复性 | 调车记录、测量工具、照片或设定说明 |
| 动态实车 | shakedown tuning、稳态圆、制动、加速、路肩、连续弯 | 数据日志、悬架位移传感器、胎温胎压、车手反馈、视频 |
| 出车后检查 | 触底痕迹、bump rubber 接触、漏油、紧固件、杆端间隙、轮胎磨耗 | post-run inspection 表、照片、维修记录 |
...
- [Jim Kasprzak: A Guide to Your Dampers](https://www.kaztechnologies.com/wp-content/uploads/A-Guide-To-Your-Dampers-Chapter-from-FSAE-Book-by-Jim-Kasprzak-Updated.pdf)，用于阻尼器速度-力曲线、低 / 中 / 高速阻尼、dyno 检查、motion ratio 和调校语言。
- [OptimumG: Of Springs and Dampers](https://optimumg.com/wp-content/uploads/2021/10/racecar-2020_11.pdf) 与 [UNCA FSAE Springs](https://sites.google.com/unca.edu/suspension/learning/vehicle-dynamics-basics/springs)，用于 ride rate、wheel rate、tire stiffness、motion ratio convention 和弹簧阻尼基础。
- [Cooper Union FSAE Suspension Design Review](https://scholars.duke.edu/publication/1426694)，用于把轮胎、几何、弹簧、阻尼和 anti-roll bar 放进并行设计流程。
- [MIT DSpace: Optimization of a Formula SAE Vehicle's Suspension Kinematics](https://dspace.mit.edu/entities/publication/552bc5c1-9705-4a17-847d-9a9a9ff27b60)，用于优化流程、目标函数、模型假设和验证边界的方法参照，不支撑参数配方、硬点或调校答案。
- [Penske: Natural Frequency, Ride Frequency, and CPM](https://www.penskeshocks.com/blog/natural-frequency-ride-frequency-cpm-in-race-suspension)，用于 aero load、dynamic ride height 和 aero-platform 边界提醒，不作为 Formula Student 必选方案。
