...
悬架设计的第一步不是打开 CAD 画 A 臂，也不是直接沿用上一代硬点，而是把“今年这台车要成为什么样的车”翻译成悬架可以执行、可以评审、可以验证的目标。公开手册中的 [快速版设计目标章节](../02-design-targets.md) 已经说明了基本输入和常见判断，本章进一步展开目标分解、跨组接口、证据记录和评审门槛。

本章要解决四个问题：

1. 如何从整车目标、比赛目标和车手反馈中提炼悬架目标。
2. 如何区分必须满足的约束 constraint、希望优化的目标 target、以及可以取舍的偏好 preference。
3. 如何让目标表既能指导几何、弹簧阻尼、仿真和结构校核，又不会公开历史赛车的具体性能数据。
4. 如何在输入变化时触发重新评审，而不是让早期假设静悄悄地污染后续设计。

这里的“目标”不是口号。合格的目标应至少说明工程含义、单位或评价方式、来源、验证方法、负责人、版本和更新触发条件。对于安全相关或结构相关内容，本章只给出公开的工程流程，不能替代规则审查、专业校核、材料数据、制造验证和实车测试。
...
公开文档应保留这种推理链，但不能公开历史赛车的精确加速、绕桩、侧向极限、车辆编号或内部目标表。可以写“提升低速弯响应”“改善弯中稳定性”“目标接近中性转向”“在规则和包络内增加可调范围”，但不要写可识别某一年车辆的精确成绩或参数组合。

从整车目标落到悬架目标时，建议先形成一张“悬架目标草表”，每一行只表达一个工程问题。例如：

| 目标类别 | 工程含义 | 悬架影响 | 证据 | 不公开内容 |
| --- | --- | --- | --- | --- |
| 操纵稳定性 handling stability | 车辆在稳态转弯、瞬态转向和制动转向组合工况下是否可预测 | 主销参数、前束变化、侧倾刚度分配、轮胎外倾控制、阻尼调校方向 | 车手反馈、视频、加速度和方向盘角日志、简化操稳模型、整车仿真 | 历史车辆编号、原始成绩、未脱敏日志 |
...
| 制动与入弯稳定 braking entry | 制动时姿态、前后轮荷变化和方向稳定性是否可控 | 抗俯仰几何、前束变化、轮胎纵滑与侧偏余量、阻尼设定 | 制动数据、俯仰姿态、轮速、车手入弯修正量 | 未公开制动系统参数和原始数据 |
| 可靠性与可维护性 reliability | 方案能否完成测试和比赛，损伤后能否快速检查和更换 | 结构裕度、杆端轴承选型、装配空间、紧固件可达性、限位保护 | 结构校核、装配评审、测试后检查记录 | 原始载荷数据表、供应商私密数据、未脱敏失效记录 |
...
公开手册只应记录这些规则如何转化为工程检查，不应把某一年规则截图、内部 scrutineering 表或未经确认的解释当成永久真理。若规则最低值和团队性能目标冲突，先满足规则和安全，再讨论是否通过车高、运动比、限位、包装或目标取舍重新设计。
...
- 加速度目标：若用 `g` 表示，应说明这是相对于重力加速度的无量纲倍数，用于目标等级或工况假设，不应公开为历史精确成绩。

如果使用早期载荷转移或频率估算，应在公式附近定义符号。例如讨论纵向载荷转移时，可以只写框架：载荷转移与整车质量 `m`、纵向加速度等级 `a_x`、质心高度 `h_CG` 和轴距 `L` 有关；其中 `m` 的单位为 kg，`a_x` 可用 g 的倍数或 m/s²，`h_CG` 与 `L` 的单位应一致。公开文档中不需要给出历史车辆的具体输入值。
...
历史悬架资料中常把目标归纳为两类：操纵稳定性 handling stability 和车辆极限 limit performance。公开文档可以保留这个工程逻辑，但不公开对应年份的精确成绩、车辆编号和目标数值。
...
| 制动稳定 | 控制制动俯仰、前束变化和轮胎复合工况余量 | 未脱敏制动系统参数和测试曲线 |
...
完成本章后，团队至少应有以下公开安全的输出物。内部版本可以包含具体数值和原始数据，但公开仓库只能保留改写后的结构、字段和方法。

| 输出物 | 最低内容 | 用途 |
| --- | --- | --- |
| 悬架设计目标表 | 目标类别、工程含义、单位或评价方式、类型、来源、证据、负责人、版本、更新触发、不公开内容 | 把整车意图转成悬架任务 |
...
建议目标表使用“公开版”和“内部版”两层。公开版保留字段、逻辑和泛化结论；内部版由团队自行管理具体数据、历史成绩和未脱敏记录。这样既能把经验转化为可维护的公开知识，也能避免泄露不应公开的历史细节。

## 常见错误

1. 直接复制旧车目标，却没有检查今年的轮胎、车架、规则、气动、制造能力和车手反馈是否变化。
2. 用精确历史成绩包装目标，公开后既不安全，也会误导读者把某一年结果当成普适答案。
3. 把“提升操稳”写成单句目标，却没有拆成稳态、瞬态、制动入弯、弯中、出弯和颠簸路面的验证问题。
4. 把偏好当成约束，例如“沿用某个布置”没有说明是制造周期限制、成本限制，还是单纯熟悉。
5. 只和车架确认硬点，不和转向、制动、传动、气动、车身、制造、测试和车手确认接口。
6. 单位和符号不清：同一个前束角有人按单轮 toe 写，有人按总前束写；同一个坐标值有人用 mm，有人用 m。
7. 早期质量、质心、轮胎模型或包络更新后，没有回头检查偏频、行程、载荷、几何和仿真。
8. 目标表只写最终结论，不记录被放弃方案和取舍理由，导致后续成员无法复盘。
9. 仿真目标过度精细，但测试条件无法验证，最后无法判断模型是否可信。
10. 公开文档中保留源表格、源图、内部编号或组合参数，使读者能反推出历史车辆。
...
| 单位与坐标系 | 所有 mm、deg、N、N·m、kg、N/mm、N·m/deg 是否定义清楚 | 参数表表头、坐标系说明、模型设置截图的公开改写描述 |
...
| 公开安全 | 是否删除历史车辆编号、原始成绩、源表、源图、精确硬点、原始载荷和未脱敏记录 | 发布前红线检查 |
...
目标章节写得越清楚，后续章节越不容易变成孤立的公式、软件截图和参数表。悬架设计的公开价值也正在这里：不是公开某台车的答案，而是公开如何把复杂约束拆成可学习、可验证、可复盘的工程流程。
