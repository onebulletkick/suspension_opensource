...
几何阶段的软件链应从“二维几何关系”走到“三维包络”和“K&C 可解释曲线”。不要把某个软件输出当终点；硬点被接受前，必须同时通过几何、包络、运动学、结构接口和版本记录。

| 技术问题 | 推荐工具 | 输入 | 输出 | 传给下一步 | 验证方式 |
| --- | --- | --- | --- | --- | --- |
| 第一版硬点推导 | AutoCAD 或 2D CAD、表格 | 坐标系、轮胎 / 轮辋边界、目标 camber、caster、KPI、trail、toe 和 steering 几何 | 二维几何草图、硬点初表、坐标和正负号说明 | CATIA / 3D CAD、Adams Car | 与手算几何、左右镜像和坐标导出结果复核 |
| 三维包络检查 | CATIA / 3D CAD | 硬点初表、轮辋、制动、转向、车架、车身、传动和维护空间 | 组合姿态干涉清单、球铰摆角、工具空间和接口问题 | 硬点修正、结构支座设计 | 检查静态、跳动、回弹、转向、侧倾和公差组合 |
| K&C 曲线检查 | Adams Car | 已核对硬点、模板、单位、轮胎半径、转向和轮跳工况 | camber gain、toe change、bump steer、roll center migration、track change 曲线 | 簧上参数、整车仿真、评审记录 | 曲线趋势解释、动画检查、坐标和符号 sanity check |
| 硬点优化 | Adams Insight、表格 | 设计变量、变量范围、目标函数、约束、包络和制造边界 | DOE / 灵敏度、优化前后对比、拒绝方案理由 | CATIA / 3D CAD、Adams Car、结构章节 | 优化结果回到三维包络和结构接口复核 |
| 版本固化 | Git / Markdown、表格 | 硬点版本、变更原因、受影响章节、接口确认状态 | 硬点变更记录、K&C 曲线包、接口 sign-off 清单 | 簧上系统、载荷模型、测试计划 | 每次硬点变化触发相关模型重跑和评审记录更新 |
