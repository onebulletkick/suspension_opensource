...
RCVD-style axis systems 或任何教材 / 软件坐标系统进入项目后，都必须先固定原点、轴向、正负方向、左右镜像和输入条件，再比较硬点或曲线。否则 camber gain、toe change、steering axis 和 roll-center migration 可能只是符号相反、参考面不同或输入被锁定方式不同，却被误判成几何优劣差异。公开文档可以讲通用约定；项目工程记录必须把实际约定写到硬点表、K&C 模型、CAD 导出和测试复盘里。
