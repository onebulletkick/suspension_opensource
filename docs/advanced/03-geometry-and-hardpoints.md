...
- 如何用公开安全的表格模板记录硬点、坐标系、单位、符号、版本和接口状态，而不发布历史赛车的硬点精确位置或私有目标值。
...
公开版硬点表只应提供字段，不提供历史坐标：

| 点位字段 | 说明 | 版本记录 |
| --- | --- | --- |
| 轴别与左右 | 前 / 后、左 / 右、是否镜像生成 | 关联整车坐标系版本 |
| 点位名称 | 例如上 A 臂前内点、下 A 臂后内点、转向拉杆外点、轮心、推杆点 | 使用项目统一命名 |
| `x` coordinate | 整车坐标下的纵向位置，单位见表头 | 内部版填写数值；公开版保留字段 |
| `y` coordinate | 整车坐标下的横向位置，单位见表头 | 内部版填写数值；公开版保留字段 |
| `z` coordinate | 整车坐标下的竖向位置，单位见表头 | 内部版填写数值；公开版保留字段 |
| 来源 | CAD、K&C 模型、优化器、测量或临时假设 | 写明可信度和更新触发 |
| 接口状态 | 车架、转向、制动、传动、气动、制造是否确认 | 未确认项不得假装冻结 |

这张表的目的不是公开项目答案，而是让读者知道合格硬点记录必须能被 CAD、多体动力学、结构校核、装配测量和测试复盘共同使用。
...
公开文档中可把侧视虚拟摆臂 side-view swing arm, `SVSA` 作为理解工具：它由侧视瞬心和轮胎接地点关系帮助解释纵向力如何引起车身俯仰趋势。`SVSA` 不是单独目标值；它必须和制动比例、驱动方式、质心高度、轮胎纵向力、阻尼和结构节点一起评审。
...
硬点优化的目标是改进可验证的车辆行为，而不是让某条曲线贴近无来源的数值。优化前必须明确设计变量、目标函数、约束、权重、工况、软件版本和退出条件。公开文档只能描述目标类别和约束逻辑，不能发布历史硬点精确点位、精确优化目标、源表格或源图。
...
K&C 是 kinematics and compliance 的缩写。早期几何章节通常先做刚体运动学 kinematics，检查连杆几何如何决定车轮运动；更成熟时还应考虑 compliance，即杆端、upright、车架和轮胎在载荷下的弹性变形如何改变 camber、toe 和轮心位置。若没有柔性数据，公开文档应写成“刚体 K&C 初筛”，并把柔性影响列为待验证项。
...
制造检查的关键是把“理论硬点”变成“可定位硬点”。公开文档可以使用字段模板，例如：定位基准、孔位类型、可调范围、测量工具、允许误差来源、装配顺序、复测方法、风险备注。不要发布历史车辆的实际孔位、夹具尺寸或内部制造图。
...
完成本章后，团队应至少形成以下输出物。内部版本可以包含具体坐标和模型文件；公开版本只保留字段、逻辑、方法和脱敏结论。

| 输出物 | 最低内容 | 公开边界 |
| --- | --- | --- |
| 几何目标说明 | 每个目标的工程含义、来源、相关轮胎或车手反馈、验证方法 | 不发布历史精确目标值 |
| 硬点表模板 | 坐标系、单位、点位字段、版本、来源、接口状态 | 不发布实际坐标和可反推历史车辆的组合数据 |
| K&C 曲线包 | camber、toe、bump steer、roll center、track change、Ackermann、anti 参数等曲线 | 可公开示意和字段；内部曲线需脱敏或重绘 |
| CATIA / 3D CAD 包络记录 | 极限姿态、干涉检查、维护空间、制造可达性 | 不发布源截图或未授权 CAD 图 |
| 优化记录 | 设计变量、目标类别、约束类别、被拒绝方案和取舍理由 | 不发布精确优化设置或源表格 |
| 接口签字表 | 车架、转向、制动、传动、气动、制造、测试确认状态 | 公开版写流程和字段，不写私有负责人或内部编号 |
| 版本变更日志 | 每次硬点变化的原因、影响章节、重新验证项 | 不公开能重构历史坐标的数值变化 |
...
10. 公开文档中保留源图、源表、精确坐标或源文件名，使读者能反推出内部车辆。
...
- 公开版文档是否删除实际坐标、精确历史目标、源表、源图和源文件名。
