...
可以公开的内容包括：经过改写的设计经验、方法论、公式框架、建模流程、评审问题、验证思路、通用软件工作流、不指向具体历史车辆的假想示例，以及确认自有或授权、经过审查和脱敏的图例。公式应作为理解力学关系和检查量纲的框架使用，并在需要时说明适用假设、单位和坐标系。

不能公开的内容包括：原始数据、可反推出历史赛车参数的表格或组合信息、具体硬点精确点位、原始载荷值、模型拟合参数、未经审查的源图、未经授权的截图、内部车辆编号、供应商私密信息和未脱敏的评审记录。来自历史资料的结论进入公开文档前，必须改写为通用工程逻辑；来自历史资料的图例进入公开文档前，必须完成图表登记、权限判断、敏感信息清理和反推检查，并在不确定时标注“待公开资料验证”或“待补充”。
...
| 10 | [10 评审清单](10-review-checklists.md) | 把设计过程变成可复盘、可交接、可公开维护的检查清单 | 阶段评审表、风险记录、待验证项 |

## 源文档覆盖摘要

本节只展示脱敏后的来源类别和公开章节落点。完整逐项覆盖审计保留在内部忽略记录中，用于维护“来源知识点 -> 章节落点 -> 公开边界”的对应关系，不进入 GitHub Pages。

| 内容来源范围 | 覆盖章节 | 公开改写重点 | 不公开内容 |
| --- | --- | --- | --- |
...
```mermaid
flowchart LR
  A["来源范围<br>历史经验 / 技术路线 / 校核记录"] --> B["公开改写<br>去除原始数值 / 改成通用逻辑"]
...
映射只说明公开改写的来源方向，不代表读者需要访问或拥有原始资料。公开章节应能独立阅读，并把无法公开确认的细节标注为待补充或待验证。
