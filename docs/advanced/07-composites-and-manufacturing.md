...
快速预览层见 [07 载荷与结构校核](../07-loads-and-structure-check.md)。本章只发布公开安全的流程、评审问题和保守判断方式，不发布私有材料供应商数值、历史铺层参数、源公式、源图表、车号或可反推出具体车辆的组合数据。
...
复材仿真的可信度首先由材料数据决定。公开资料、供应商数据、历史经验和实测 coupon 之间的证据等级不同，不能混在一起使用而不说明来源。团队应建立材料数据表，但公开仓库只保留字段和判断逻辑，不发布私有供应商值或内部测试结果。
...
材料 / 铺层字段模板建议至少包含以下项。公开文档只写字段、单位和坐标定义；内部工程包再填写具体数值、来源和版本。
...
公开文档中不要发布历史铺层表、供应商厚度值或能反推出实际车辆零件的参数组合。可以发布字段模板、检查逻辑和“需要在内部设计包中填写”的项目。
...
对于 Hashin、Puck-type、损伤演化或用户材料模型，要记录模型版本、适用假设、输出变量含义和后处理脚本。若内部使用 Fortran、VUMAT / UMAT 或其它用户子程序，公开文档只说明它属于 user material implementation，不复制源代码、状态变量编号或删除阈值；读者需要的是可复核的工程方法，而不是内部实现。

## 图示：复材建模、网格和结果读法

下面的图例来自已清理的历史复材校核资料，用来说明复材件在公开手册中应该如何呈现：展示铺层、网格、连接区和后处理思路，但不公开私有材料参数、真实铺层表、载荷数值或源模型文件。

![复合材料管件的层合板截面示意](../assets/figures/composites/composite-tube-laminate-section.png)

截面图能帮助读者理解 laminate 不是单一实体材料。铺层厚度、方向、顺序和局部补强都会影响刚度、失效模式和制造风险；公开版本只保留层状结构和方向概念，不给出历史铺层参数。
...
复材校核建议形成以下公开安全的输出物模板：

| 输出物 | 最低内容 | 公开版写法 |
| --- | --- | --- |
| 复材设计说明 | 零件功能、载荷路径、铺层原则、连接策略和制造工艺 | 说明方法和字段，不发布历史铺层表 |
| 材料数据记录 | 数据来源、allowables 类型、适用工艺、环境和折减策略 | 不发布私有供应商值或内部测试数值 |
| Abaqus 设置说明 | 单元、材料、坐标、边界、接触、连接、网格和后处理 | 描述可复核流程；可发布已清理的自有模型图例，不复制源模型、私有材料值或脚本细节 |
| 失效模式评审 | fiber、matrix、shear、delamination、crushing、buckling、pull-out 风险 | 用模式和位置类型表达；云图只说明读图方式，不公开可反推出车辆或材料的组合数据 |
| 制造质量计划 | 材料批次、铺层检查、固化记录、修边钻孔、首件和返修限制 | 保留检查项和判断逻辑 |
| coupon / 样件计划 | 测试目的、加载方式、记录项目、通过标准和边界 | 不公开内部试验数据，保留验证思路 |
| 释放结论 | 当前证据等级、未覆盖风险、停用标准和下一步动作 | 使用保守语言，不写过度承诺 |

内部工程包可以包含具体数值、图纸、铺层表和测试数据；公开手册应保留可学习、可迁移、不可识别具体车辆的工程逻辑。
...
- 公开文档中暴露私有铺层、材料参数、源公式、源图表或车辆识别信息。
...
- 公开文档是否删除了私有材料值、铺层参数、源图表、车号和内部文件名。
