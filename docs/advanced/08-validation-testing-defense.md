...
公开仓库只发布通用流程、字段模板、检查逻辑和保守的工程经验。不发布原始数据文件、精确测试值、比赛专用内部记录、未脱敏故障照片、历史车辆编号、源图表或能反推出某一年车辆参数的组合信息。
...
建议使用以下字段。字段名可以直接进入内部测试计划；公开版本保留字段和示例写法，不填写原始数值。
...
矩阵中的“通过标准”应由团队依据规则、安全策略、车辆阶段和测试条件定义。公开文档不提供通用阈值，因为阈值与车辆质量、轮胎、场地、传感器、车手水平和风险策略有关。若标准尚未建立，应写成“待验证：通过标准定义”，而不是用模糊词替代。
...
测试计划必须包含 issue grading 和 stop condition。悬架涉及转向、制动、结构连接和轮胎接地，任何安全相关异常都应优先于性能目标。文档应使用保守语言：公开手册不能替代规则检查、专业结构评审、现场安全负责人判断和实车测试流程。
...
| 数据结果 | 对应数据通道、图表和关键观察，不写原始私有数值到公开文档 |
| 车手反馈 | 保留现象描述，并与推断分开 |
...
公开文档可以给出调校字段和判断逻辑，但不公开内部调校配方、比赛策略、精确胎压、阻尼档位组合、赛道分段成绩或未脱敏原始日志。
...
答辩证据包可以包括公开安全的目标分解、验证矩阵、测试流程、通道列表、对比图模板、问题分级、调校日志结构、模型修正记录和下一季问题表。内部答辩材料可以有具体数值，但对外开源文档应只保留方法和脱敏示例。
...
| 数据和脚本索引 | 原始数据内部存放位置、处理脚本版本、通道定义、公开不可发布内容 |
...
公开版本可以保留传承结构、字段和经验类型；内部版本保存具体数据、故障照片、比赛记录、未脱敏日志和原始文件索引。两者要分开维护，避免把内部证据误提交到公开仓库。
...
| 输出物 | 最低内容 | 公共发布边界 |
| --- | --- | --- |
| 验证矩阵 validation matrix | 目标、仿真证据、测试方法、数据通道、通过标准、未验证风险 | 可发布字段结构和脱敏示例，不发布精确值 |
| 静态检查表 static checklist | 设定、紧固、干涉、行程、胎压、传感器标定和数据通道 | 可发布检查逻辑，不发布车辆专属设定 |
| 测试日志 test log | 测试方法、车辆状态、每轮变量、车手反馈、数据文件和异常 | 可发布模板，不发布原始数据文件和内部记录 |
| 数据通道表 channel map | 通道名、单位、方向、采样率、标定状态、用途 | 可发布通用字段，不发布私有硬件配置细节 |
| 相关性记录 correlation note | 仿真与实测一致处、差异处、原因排序、模型修正和复核计划 | 可发布方法，不发布可识别历史车辆的数据图 |
| 调校记录 setup log | 本轮变量、预期、数据、反馈、下车检查、结论强度和下一步 | 可发布字段，不发布比赛策略和调校配方 |
| 问题清单 issue list | 问题等级、触发条件、证据、动作、复测条件和责任人 | 可发布分级逻辑，不发布未脱敏故障材料 |
| 答辩证据包 defense pack | 目标、推理、证据、限制、已验证项、未确定项、下一步 | 可发布结构，不发布内部比赛记录 |
| 下一季传承包 handover pack | 可信结论、失败假设、未验证风险、数据索引和优先级 | 可发布传承原则，不发布原始文件路径和私有资料 |
...
| 公开边界是否清楚 | 没有原始数据、精确测试值、内部记录、源图或可识别历史车辆信息 |

对安全相关结论要保持保守：公开手册只能提供流程和检查逻辑，不能替代规则、现场安全流程、结构校核、制造质量控制和专业评审。若证据不足，正确做法是标记为待验证，而不是把“未发现问题”写成“已经证明安全”。
...
当团队不确定某个结论是否能公开时，应优先保留流程、字段和通用判断，删除原始数值、内部记录和可识别来源。公开文档的目标是教会读者如何建立验证闭环，而不是发布某支车队的私有测试档案。
