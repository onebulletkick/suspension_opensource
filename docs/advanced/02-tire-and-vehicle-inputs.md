...
轮胎阶段的软件任务是把“轮胎行为”转成后续几何、簧上系统和整车仿真都能读取的输入。工具选择可以不同，但交付物必须说明数据覆盖、坐标约定、模型边界和相关性计划。

| 技术问题 | 推荐工具 | 输入 | 输出 | 传给下一步 | 验证方式 |
| --- | --- | --- | --- | --- | --- |
| 数据清单与单位统一 | 表格、MATLAB / Python | 轮胎数据字段、轮辋、胎压、温度、载荷、滑移角和滑移率定义 | 数据覆盖矩阵、单位表、异常段说明 | 拟合工具、整车输入表 | 抽样画图、左右轮符号检查、数量级检查 |
| 轮胎模型拟合 | Optimum Tire、Adams Tire Data and Fitting Tool、MATLAB / Python | 清洗后的纯侧向、纯纵向和联合工况数据，模型形式和拟合权重 | Magic Formula / Pacejka-style 或查表模型、残差趋势、适用窗口 | Adams Car、CarSim / Simulink、脚本分析 | 残差审查、未覆盖工况标注、导入后曲线复核 |
| 候选轮胎对比 | 表格、MATLAB / Python、轮胎可视化工具 | 候选轮胎尺寸、刚度、峰值力、载荷敏感性、采购和包络边界 | 候选对比表、风险说明、推荐和备选方案 | 设计目标、几何包络、弹簧阻尼参数 | 用相同工况和坐标比较，不跨数据窗口泛化 |
| 动态轮荷输入 | 表格、MATLAB / Python | 质量、轴距、轮距、质心高度、气动假设、制动 / 驱动边界 | 轴荷和单轮轮荷估算、输入版本、更新触发条件 | 簧上系统、仿真、载荷工况 | 与简化载荷转移公式、整车模型和实车角重互相复核 |
| 实车相关性更新 | Race Studio / AIM、MATLAB / Python、Git / Markdown | 胎温胎压、加速 / 制动 / 转向数据、车手反馈、测试日志 | 模型修正记录、相关性差异、下一轮测试计划 | 轮胎模型版本、调校记录、答辩证据 | 同一车辆状态和同一测试条件下复测趋势 |
