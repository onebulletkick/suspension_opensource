...
公开文档只描述通用流程和评审逻辑。任何原始测试曲线、具体型号对比、私有轮胎数据、拟合参数、源图和历史车辆输入组合，都应留在团队内部资料中，不应进入公开仓库。
...
轮胎数据进入计算前，必须先统一坐标系、符号、单位和参考方向。不同测试机构、教材和仿真软件可能采用不同正方向。SAE 与 ISO-style 坐标表达之间常见差异包括侧向力方向、侧偏角 slip angle 正方向、外倾 camber 正方向、回正力矩 aligning moment 正方向，以及纵向滑移 slip ratio 的定义方式。公开文档不需要绑定某个商业软件的内部格式，但必须把本项目采用的约定写清楚。
...
下面的模板用于项目公开文档或模型说明。它不声称某一种约定天然正确，而是要求团队把自己的约定写成可检查的条目。填写时可以把“本项目采用”替换为团队内部版本；公开仓库只保留约定形式，不保留会暴露历史车辆的具体数据。
...
常见软件或模型名可以作为公开示例，例如 MF5.2、PAC2002、Pacejka-style 参数化模型、Optimum Tire 类拟合工具、Adams Car 的 Tire Data and Fitting Tool，或 MATLAB / Python 自写拟合脚本。它们的作用是组织数据、拟合曲线和导出软件接口，不是替代数据质量、坐标约定、残差检查和实车对标 correlation。公开文档只讨论工具类别和评审逻辑，不发布内部模型文件、软件截图或精确参数。
...
拟合不是把数据丢进软件后导出参数文件。它是一套可复现的数据处理、坐标转换、模型选择、权重设置、残差评审和版本管理流程。每一步都可能引入偏差，公开手册应教读者记录流程，而不是复制某个历史项目的工具截图或私有设置。
...
公开文档可以说明轮胎比较的维度，但不应发布具体历史型号的私有对比值或可识别源数据的曲线。比较的目的不是宣布某个轮胎“绝对最好”，而是在给定车辆目标、规则、预算和测试能力下，选择最适合的设计边界。
...
完成本章后，团队至少应形成以下输出物。公开仓库可以保留字段、流程和泛化结论；内部工程库可以保存具体数据、模型文件和测试记录。
...
10. 公开文档中泄露原始曲线、具体型号对比、私有测试值或可以反推历史车辆的组合信息。
...
模型可信度至少应满足这些公开安全的检查标准：残差图按载荷、外倾、侧偏角、滑移率和输出量分箱查看；设计窗口 residual 单独评审；没有未解释的符号反转；没有未解释的力、力矩或导出曲线不连续；数据量允许时做 holdout 或 cross-validation；实车相关性接受标准用定性工程语言表达，例如“主要趋势一致、参数变化方向一致、异常差异可追溯且已记录”，而不是承诺某个公开数值门槛。
