...
快速预览层见 [07 载荷与结构校核](../07-loads-and-structure-check.md)。本章只给公开安全的流程、公式框架、字段模板和评审逻辑，不发布历史载荷数据表、历史应力结果、源图、源表、车号或可反推出具体车辆的参数组合。所有结论都应写成“在当前假设下支持下一步设计 / 制造 / 测试评审”，不能把 FEA 单独当作安全认证。
...
下面几张图来自已清理的历史结构校核图例，用来补足“多体导力 - 网格 - 边界条件 - 结果解释”的视觉链条。它们只用于说明公开手册中的评审方法；颜色、形状和局部热点不代表可复用的设计答案，任何真实项目仍需回到自己的载荷、材料、边界和验证数据。

![多体模型中的悬架载荷路径与接口力示意](../assets/figures/metal/metal-load-path-mbd-overview.png)

多体模型视图适合用来检查力从轮端、upright、A 臂、推杆 / 拉杆和车架支座进入结构的路径。公开版本保留载荷路径和接口力的概念，不保留真实工况数值、模型文件名或历史车辆参数。
...
气动载荷 aero load 在早期可按轴向分配写成假设输入，例如 `F_z,aero_front(v)` 和 `F_z,aero_rear(v)`。公开文档中应只说明函数关系、来源和验证状态，不发布私有气动图谱。若没有可靠气动模型，应把气动作为保守假设或敏感性参数，并标注是否考虑俯仰、侧倾、车高和偏航角影响。
...
这张表是字段模板，不是载荷答案。内部版本可以填数值；公开版本应保留方法、来源和复核逻辑。
...
| 输出物 | 最低内容 | 公开版写法 |
| --- | --- | --- |
| 载荷工况表 | 工况、目的、输入、载荷来源、坐标系、输出、复核方法、版本 | 只发布字段模板和示例逻辑，不发布历史载荷值 |
| 多体导力记录 | 模型版本、硬点版本、工况、关节定义、峰值时刻、后处理脚本 | 描述流程和检查项，不发布可识别车辆的结果表 |
| 自由体图复核 | 力平衡、力矩平衡、符号、数量级和载荷路径解释 | 可用自绘示意或 Mermaid 流程，不复制源图 |
| FEA 设置说明 | 几何、材料、接触、连接、约束、网格、求解器和后处理 | 写通用设置清单；可发布已清理的自有 FEA 图例，不发布原始模型、载荷表或路径信息 |
| 结果解释报告 | 应力、位移、稳定性、疲劳风险、制造风险和结论等级 | 用保守语言说明方法和判断；图例只说明读图方式，不作为可复用设计值 |
| 设计修正记录 | 修改原因、版本差异、重算工况、接口影响和释放状态 | 总结经验，不发布历史车辆参数 |
| 物理检查记录 | 首件、装配、扭矩、裂纹、松动、磨损和测试后检查 | 发布检查模板，不发布内部事故或未授权照片 |

内部文件可以有具体数值，公开仓库只应保留可复用的方法、表头、评审问题和待验证说明。
