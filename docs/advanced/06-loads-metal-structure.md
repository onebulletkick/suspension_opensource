...
## MBD-to-FEA 载荷转换与 boundary package
...
可直接采用下面的公开模板，项目版再补真实编号和数值：

| boundary package 字段 | 应记录内容 | 放行前检查 |
| --- | --- | --- |
| `case_id` | 教学编号、工况名称、车辆状态和输入版本 | 是否能追溯到同一版 hardpoints、质量、轮胎和弹簧阻尼 |
| `peak_event` | 峰值时刻、取峰规则、是否为同一时间向量 | 是否把不同时间的峰值机械叠加 |
| `force_source` | joint / marker / contact patch / rod axial force 的来源定义 | 输出对象和作用对象是否方向相反 |
| `coordinate_transform` | 原始坐标、目标坐标、符号约定和转换公式或脚本版本 | 左右件镜像、toe / steering、bump / rebound 正方向是否一致 |
| `fea_application` | 载荷对象、作用点、约束对象、coupling / bearing / contact 的分配方式 | 反力平衡、力矩臂和边界刚度是否合理 |
| `uncovered_risk` | 未覆盖的冲击、疲劳、柔度、温度、制造或测试风险 | 结论是否写成当前假设下的设计支持，而不是安全证明 |
